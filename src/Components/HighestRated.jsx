...
const HighestRated = () => {
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/highest-rated-games');
          ...
          games.map((game) => (
            <div key={game._id} className="bg-white shadow-lg rounded-lg p-5">
              ...
              <div className="flex justify-between items-center">
                ...
                <button className="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600">Explore Details</button>
              </div>
            </div>
          ))
        ) : (
